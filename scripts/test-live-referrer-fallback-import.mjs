...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (path.includes('/crm/v3/objects/contacts/search')) {
    return Response.json({
      results: [{
        id: 'contact-ivan-live',
        properties: { firstname: 'Ivan', lastname: 'Duenas', email: 'ivan@example.com' },
      }],
    })
  }
  if (path.includes('/crm/v4/objects/contacts/contact-ivan-live/associations/companies')) {
    return Response.json({ results: [{ toObjectId: 'company-ivan-live' }] })
  }
  if (path.includes('/crm/v3/objects/deals') && options.method === 'POST') {
    createDealPayload = JSON.parse(options.body)
    return Response.json({ id: 'deal-ivan-live' })
  }
...
try {
  ...
  const result = await runImportRows({
    ...
    rows: [{
      name: 'JOB-IVAN-LIVE',
      dealName: 'Ivan Duenas - JOB-IVAN-LIVE',
      referrer: 'Ivan Duenas',
      salesPerson: '',
      pipeline: 'Water',
      status: 'Sold',
  ...
  assert.equal(result.summary.held, 0, 'live HubSpot contact fallback should prevent holding the deal')
  assert.equal(result.summary.created, 1, 'deal should be created after live contact fallback')
  assert.equal(createDealPayload.associations[0].to.id, 'contact-ivan-live')
  assert.equal(createDealPayload.associations[1].to.id, 'company-ivan-live')
  assert.equal(supabase.db.upserts.hs_cached_contacts?.[0]?.hubspot_id, 'contact-ivan-live')
} finally {
