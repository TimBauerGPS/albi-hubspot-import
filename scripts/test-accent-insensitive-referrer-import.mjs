...
function createSupabaseMock() {
  const db = {
    tables: {
      ...
      hs_cached_contacts: [{
        user_id: 'user-1',
        hubspot_id: 'contact-ivan',
        first_name: 'Ivan',
        last_name: 'Dueñas',
        company_hubspot_id: 'company-ivan',
      }],
      hs_cached_companies: [],
      hs_held_deals: [],
      hs_imports: [],
...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (path.includes('/crm/v3/objects/deals') && options.method === 'POST') {
    ...
    return Response.json({ id: 'deal-ivan' })
  }
...
try {
  ...
  assert.equal(result.summary.held, 0, 'accentless referrer should not be held when HubSpot has an accented match')
  assert.equal(result.summary.created, 1, 'deal should be created when accented contact is matched')
  assert.equal(createDealPayload.associations[0].to.id, 'contact-ivan')
  assert.equal(createDealPayload.associations[1].to.id, 'company-ivan')
} finally {
